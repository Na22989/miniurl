...
public class StatusServiceImpl implements StatusService {
    ...
    public LinkStatusVO getLinkStatus(Long linkId, Long userId) {
        ...
        // 2. 一次查询校验存在性 + 归属（越权/不存在统一返回 LINK_NOT_FOUND，不暴露存在性）
        ...
        // 3. 总 PV：click_count 是权威值（Redis INCR → 定时落库），link_access_log 是异步尽力而为
        long pv = link.getClickCount() == null ? 0L : link.getClickCount();

        // 4. 总 UV / 今日 PV / 今日 UV（都走 idx_link_id_time 索引）
